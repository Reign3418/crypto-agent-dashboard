 ...
 * @param {number|null} minTradeSizeOverride - Tank-calibrated min trade size (default $15)
 */
export function runNumNum({ side, usdAmount, currentPrice, buyPrice, numNumFloor, numNumStopLoss, minTradeSizeOverride, isFullLiquidation }) {
  ...
  // Dynamic min trade size — Tank calibrates based on liquid capital. Hard floor: $15.
  const minTradeUsd  = minTradeSizeOverride != null
    ? Math.max(parseFloat(minTradeSizeOverride), MIN_TRADE_SIZE_USD)
    : MIN_TRADE_SIZE_USD;
  ...
  const calibrationNote = numNumFloor != null
    ? ` [Tank floor: ${minProfitPct.toFixed(1)}% | stop: ${stopLossPct.toFixed(1)}% | min: $${minTradeUsd}]`
    : ` [default floor: ${minProfitPct.toFixed(1)}% | min: $${minTradeUsd}]`;

  // ── BUY CHECK ─────────────────────────────────────────────────────────────
  if (side === 'buy') {
    if (usdAmount < minTradeUsd) {
      return {
        approved: false,
        reason: `NumNum REJECT: Trade size $${usdAmount.toFixed(2)} is below the $${minTradeUsd} minimum. Fee overhead makes this trade unviable.`,
  ...
  if (side === 'sell') {
    ...
    // FULL LIQUIDATION BYPASS
    // When Tank has authorized a full position exit (isFullLiquidation=true),
    // the profit floor is irrelevant — you exit with what you have.
    // The stop-loss above is still the absolute safety net.
    // Fee drag is accepted as the cost of freeing trapped capital.
    if (isFullLiquidation) {
      return {
        approved: true,
        reason: `NumNum APPROVE (FULL LIQUIDATION): Exiting entire position at ${currentPctFromBuy.toFixed(2)}% from buy. Profit floor waived by Tank for controlled exit. Net after fees: ~${netProfitPct.toFixed(2)}%.${calibrationNote}`,
        currentPctFromBuy: parseFloat(currentPctFromBuy.toFixed(4)),
        breakEvenPrice: parseFloat(breakEvenPrice.toFixed(6)),
        targetSellPrice: parseFloat(targetSellPrice.toFixed(6)),
        feeDragPct,
