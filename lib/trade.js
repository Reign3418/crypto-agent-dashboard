...
export async function executeTrade(symbol, side, usdAmount) {
  ...
  const ALLOWED_ASSETS = ['BTC', 'ETH', 'SOL', 'XRP'];
  ...
  try {
    ...
    const avgPrice = data.avg_execution_price || limitPrice;
    
    // Calculate the TRUE exact USD volume of the trade that executed
    const trueUsdVol = parseFloat(execAmount) * parseFloat(avgPrice);

    await logAction(`✅ Trade Executed: ${side.toUpperCase()} ${execAmount} ${symbol} for ~$${trueUsdVol.toFixed(4)} (IOC). Fee: ${feeAmount} ${feeCurrency}`);
    ...
    try {
      ...
      if (side === 'buy') {
        ...
        // Fall back to currentPrice (not limitPrice) if the exchange doesn't return it.
