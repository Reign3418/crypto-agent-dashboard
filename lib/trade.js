...
export async function executeTrade(symbol, side, usdAmount) {
  ...
  // ── Dynamic precision: Gemini requires amounts rounded to the symbol's
  // min_order_increment tick size. Using too many decimal places returns
  // "Invalid quantity" (e.g. LTC = 0.00001 → 5dp max, DOGE = 1 → 0dp).
  // Fallback map covers all 9 supported assets if the API call fails.
  const PRECISION_FALLBACK = {
    BTC: 5, ETH: 5, SOL: 2, XRP: 2,
    LINK: 2, DOGE: 0, LTC: 5, AVAX: 2, BCH: 5,
  };
  let decimals = PRECISION_FALLBACK[symbol.toUpperCase()] ?? 5;
  try {
    const detailRes = await fetch(`https://api.gemini.com/v1/symbols/details/${tickerSymbol}`);
    if (detailRes.ok) {
      const details = await detailRes.json();
      if (details.min_order_increment) {
        const inc = parseFloat(details.min_order_increment);
        decimals = inc < 1 ? Math.round(-Math.log10(inc)) : 0;
      }
    }
  } catch (_) { /* use fallback */ }

  assetAmount = parseFloat(assetAmount.toFixed(decimals));
