  ...
  try {
    ...
    do {
      const response = await ddbDocClient.send(
        new QueryCommand({
          TableName: TABLE_NAME,
          KeyConditionExpression: "pk = :pk AND sk >= :epoch",
          ExpressionAttributeValues: { ":pk": "AGENT_LOG", ":epoch": activeEraEpoch.toString() },
    ...
    allLogs.forEach(log => {
      ...
      if (text.includes("✅ Trade Executed:")) {
        ...
        let feeAmount = feeMatch ? parseFloat(feeMatch[1]) : 0;
        
        // Retroactive fee correction for old logs (0.4%)
        if (feeAmount === 0 && usdAmount > 0) {
            feeAmount = usdAmount * 0.004;
        }
    ...
    // --- LIVE PORTFOLIO BALANCES + REAL COST BASIS ---
    let openPositionsDetailed = {};
    let totalUnrealizedPlUsd = 0;
    try {
      const { getPortfolioBalances } = await import('./trade.js');
      const balances = await getPortfolioBalances();
      // Pull the stored buy prices that were saved at execution time
      const savedPositions = settings.openPositions || {};
      
      for (const [sym, pos] of Object.entries(balances)) {
        // Filter out USD and microscopic dust (< $1.00) to prevent AI hallucinations
        if (sym === 'USD' || sym === 'GUSD' || pos.notional < 1.00) continue;
        
        const currentPrice = pos.amount > 0 ? pos.notional / pos.amount : 0;
        const currentValueUsd = pos.notional;

        // Use the real fill price saved by trade.js at execution time.
        // Fall back to current price only if no record exists (legacy positions).
        const savedPos = savedPositions[sym];
        const currentPrice = pos.amount > 0 ? pos.notional / pos.amount : 0;
        const currentValueUsd = pos.notional;

        // Priority order for buyPrice:
        // 1. Stored weighted-average (from trade.js or reconcile) — most accurate
        // 2. Current price — conservative fallback only
        const buyPrice = savedPos?.buyPrice ? parseFloat(savedPos.buyPrice) : currentPrice;

        // Priority order for costBasisUsd:
        // 1. Stored costBasisUsd from DynamoDB (written by reconcile/trade.js with correct amount)
        // 2. buyPrice × actual exchange balance — never use savedPos.amount (may be stale single-tranche)
        const costBasisUsd = savedPos?.costBasisUsd
          ? parseFloat(savedPos.costBasisUsd)
          : buyPrice * pos.amount;

        const unrealizedPlUsd = currentValueUsd - costBasisUsd;

        totalUnrealizedPlUsd += unrealizedPlUsd;

        openPositionsDetailed[sym] = {
          amountHeld: pos.amount,
          currentPrice,
          currentValueUsd,
          buyPrice,           // show the actual fill price
          costBasisUsd,       // what we actually paid
          unrealizedPlUsd: parseFloat(unrealizedPlUsd.toFixed(4)),
        };
      }
    } catch(e) { console.warn("Failed to fetch live balances for audit", e); }
    ...
    return {
      ...
      totalUnrealizedPlUsd: parseFloat(totalUnrealizedPlUsd.toFixed(2)),
      openPositions: openPositionsDetailed,
      eraName: settings.activeEraName || "Aegis",
      _systemNote: "Dust positions (< $1.00 USD) and manually closed historical trades are intentionally omitted from openPositions. Do not flag totalBuys > totalSells as an error if openPositions is empty. IMPORTANT ACCOUNTING RULE: If any coinStat entry shows sells > 0 but buyVol = 0 (example: BTC with 1 sell and $0 buy volume), this means the asset was acquired OUTSIDE this system by the human operator and then sold by CIPHER. The acquisition cost is NOT recorded in this system. Do NOT calculate a realized gain from such a sell, do NOT flag it as a capital tracking error, and do NOT include it in net P&L calculations. Treat it as a one-time anomaly and note it explicitly in your audit.",
      coinStats: Object.fromEntries(
        Object.entries(coinStats).map(([coin, stat]) => [
          coin,
          {
            ...stat,
            buyVol: parseFloat(stat.buyVol.toFixed(2)),
            sellVol: parseFloat(stat.sellVol.toFixed(2)),
            feesCents: Math.round(stat.fees * 100)
          }
        ])
      )
    };
