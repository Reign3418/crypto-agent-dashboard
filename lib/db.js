...
export async function getDeepDiveAnalysis() {
  ...
  try {
    ...
    const settingsItem = settingsResponse.Items?.[0] || {};
    const openPositions = settingsItem.openPositions || {};
    let totalUnrealizedPlUsd = 0;
    
    // Fetch live prices to calculate unrealized P/L
    let priceMap = {};
    try {
      const priceRes = await fetch('https://api.gemini.com/v1/pricefeed');
      if (priceRes.ok) {
        const prices = await priceRes.json();
        prices.forEach(p => priceMap[p.pair.toUpperCase()] = parseFloat(p.price));
      }
    } catch(e) { console.warn("Failed to fetch live prices for audit", e); }

    const openPositionsDetailed = {};
    for (const [sym, pos] of Object.entries(openPositions)) {
       const ticker = `${sym}USD`.toUpperCase();
       const currentPrice = priceMap[ticker] || pos.buyPrice;
       const costBasis = pos.amount * pos.buyPrice;
       const currentValue = pos.amount * currentPrice;
       const unrealizedPl = currentValue - costBasis;
       totalUnrealizedPlUsd += unrealizedPl;
       ...
       openPositionsDetailed[sym] = {
         ...
         unrealizedPlUsd: parseFloat(unrealizedPl.toFixed(4))
       };
    }
