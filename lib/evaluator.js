...
export async function runEvaluation({ tickerMap, scoutReport = [] }) {
  ...
  for (const strategy of active) {
    ...
    if (isTriggered) {
      ...
      const actionType = strategy.action?.type || 'alert';
      const usdAmount = strategy.action?.amount || 0;
      
      await logAction(
        `⚡ Strategy triggered: "${strategy.name}" (${sym}) — Action: ${actionType.toUpperCase()}${usdAmount ? ` $${usdAmount}` : ''}`,
        true
      );
      await markStrategyTriggered(strategy.id, strategy.triggerCount);

      // Phase 6: Autopilot Execution
      if ((actionType === 'buy' || actionType === 'sell') && usdAmount > 0) {
        try {
          await logAction(`🤖 Autopilot executing ${actionType.toUpperCase()} for $${usdAmount} of ${sym}...`);
          const { executeTrade } = await import('./trade.js');
          await executeTrade(sym, actionType, usdAmount);
        } catch (error) {
          console.error('[Evaluator Trade Error]:', error);
          await logAction(`❌ Autopilot failed to execute trade: ${error.message}`);
        }
      }
    }
  }
