...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── STEP 5: 12H Macro Ledger + TANK fallback (every 3h if tank-cron missed) ─
    // Tank runs via tank-cron (0 */3 * * *). The gate here is a FALLBACK ONLY
    // in case tank-cron missed a run. Uses THREE_HR to match the new cadence.
    if (timeLeft() > 15000 && now - timestamps.last12HTime >= THREE_HR) {
      try {
        const { default: rollupHandler } = await import('./rollup.js');
